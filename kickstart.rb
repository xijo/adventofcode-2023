...
unless File.exists?("#{folder}/part2.rb")
  File.open("#{folder}/part2.rb", "w") { |f| f.write blueprint }
end

unless File.exists?("#{folder}/example")
  example = Nokogiri::XML(URI.open("https://adventofcode.com/2023/day/#{day}").read).css('pre').first.text
  File.open("#{folder}/example", "w") { |f| f.write example }
end

input = URI.open("https://adventofcode.com/2023/day/#{day}/input", "Cookie" => File.read('.session')).read
